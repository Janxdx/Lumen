...
import { db, deleteBook, requestPersistence, type BookRecord, type ProgressRecord } from '../db';
...
export const useLibrary = create<LibraryState>((set, get) => ({
  ...
  async saveProgress(p) {
    ...
    if (p.percent >= 0.985) {
      ...
      if (book && !book.finishedAt) {
        await db.books.update(p.bookId, { finishedAt: Date.now() });
        set((s) => ({
          books: s.books.map((b) =>
            b.id === p.bookId ? { ...b, finishedAt: Date.now() } : b
          ),
        }));
      }
    }
  ...
  async recordSession(s) {
    ...
    const id = await db.sessions.add(s);
    set((state) => ({ sessions: [...state.sessions, { ...s, id: id as number }] }));
  },
}));
